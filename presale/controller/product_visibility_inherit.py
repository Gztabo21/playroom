...
class ProductVisibilityCon(WebsiteSale):
    ...
    def shop(self, page=0, category=None, search="", ppg=False, **post):
        ...
        user = (
            request.env["res.users"].sudo().search([("id", "=", request.env.user.id)])
        )
        if not user:
            ...
            if mode in ["product_only", "brand_only"]:
        ...
        if not available_categ and not available_products:
            return super(ProductVisibilityCon, self).shop(
        ...
        if available_products:
            domain_pro = self.reset_domain(
                search, category, available_products, attrib_values
            )
        ...
        if not category:
            domain = self.reset_domain(
                search, available_categ, available_products, attrib_values
            )
    ...
    def products_autocomplete(self, term, options={}, **kwargs):
        ...
        if not available_products and not available_categ:
            domain = self._get_search_domain(
                term, category, attrib_values, display_description
            )
        else:
            domain = self.reset_domain(
                term,
        ...
        products = ProductTemplate.search(
            domain, limit=min(20, options.get("limit", 5)), order=order
        )
