...
class PartnerInfoExtended(models.Model):
    ...
    xidentification = fields.Char(
        "Document Number", store=True, help="Enter the Identification Number"
    )
    ...
    personType = fields.Selection(
        [("1", "Natural"), ("2", "Juridical")], "Type of Person", default="1"
    )
    ...
    change_country = fields.Boolean(
        string="Change Country / Department?", default=True, store=False
    )
    ...
    def onchange_location(self):
        ...
        return {"domain": {domain: [("id", "in", id_domain)]}, "value": {domain: ""}}
    ...
    def _check_ident(self):
        ...
        for item in self:
            if item.doctype is not "1":
                msg = _(
                    "Error! Number of digits in Identification number must be"
                    "between 2 and 12"
                )
    ...
    def _check_ident_num(self):
        ...
        for item in self:
            if item.doctype is not "1":
                if (
                    item.xidentification is not False
                    and item.doctype != "21"
                    and item.doctype != "41"
                ):
                    if re.match("^[0-9]+$", item.xidentification) is None:
                        msg = _("Error! Identification number can only " "have numbers")
                        raise exceptions.ValidationError(msg)
