...
class IrSequenceInherit(models.Model):
	...
	sequence_dian_type = fields.Selection(
		DIAN_TYPE, "Type", required=True, default="invoice_computer_generated"
	)
	dian_resolution_ids = fields.One2many(
		"ir.sequence.dian_resolution", "sequence_id", "DIAN Resolutions"
	)
	...
	def check_active_resolution(self, sequence_id):
		...
		dian_resolutions_sequences_ids = self.search(
			[("use_dian_control", "=", True), ("id", "=", sequence_id)]
		)
			...
			if record:
				...
				if len(record.dian_resolution_ids) > 1:
					...
					for resolution in record.dian_resolution_ids:
						if (
							resolution.number_next_actual >= resolution.number_from
							and resolution.number_next_actual <= resolution.number_to
							and actual_date <= resolution.date_to
						):
				...
				if len(record.dian_resolution_ids) > 1:
					...
					for resolution in record.dian_resolution_ids:
						...
						if (
							...
							and resolution.number_next_actual <= resolution.number_to
							and actual_date <= resolution.date_to
							and resolution.active_resolution
						):
							continue
					...
					for resolution in record.dian_resolution_ids:
						...
						if (
							...
							resolution.write({"active_resolution": True})
	...
	def _next(self, sequence_date=None):
		...
		seq_dian_actual = self.env["ir.sequence.dian_resolution"].search(
			[("sequence_id", "=", self.id), ("active_resolution", "=", True)], limit=1
		)
		...
		if seq_dian_actual.exists():
			...
			if seq_dian_actual["number_next"] - 1 > seq_dian_actual["number_to"]:
				seq_dian_next = self.env["ir.sequence.dian_resolution"].search(
					[("sequence_id", "=", self.id), ("active_resolution", "=", True)],
					limit=1,
					offset=1,
				)
		...
		if self.use_dian_control:
			...
			if _active_resolution > 1:
				raise ValidationError(
					_("The system needs only one active DIAN resolution")
				)

			if _active_resolution == 0:
				raise ValidationError(
					_("The system needs at least one active DIAN resolution")
				)
