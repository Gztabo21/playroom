...
class InsTrialBalancePartners(models.TransientModel):
    _name = 'ins.trial.balance.partners'
    ...
    def write(self, vals):
        ...
        if vals.get('journal_ids'):
            vals.update({'journal_ids': [(4, j)
                        for j in vals.get('journal_ids')]})
        if vals.get('journal_ids') == []:
        ...
        if vals.get('analytic_ids'):
            vals.update({'analytic_ids': [(4, j)
                        for j in vals.get('analytic_ids')]})
        if vals.get('analytic_ids') == []:
            vals.update({'analytic_ids': [(5,)]})

        ret = super(InsTrialBalancePartners, self).write(vals)
        return ret
    ...
    def process_filters(self, data):
        ...
        if data.get('partner_ids', []):
            filters['partners'] = self.env['res.partner'].browse(
                data.get('partners_ids', [])).mapped('vat')
        else:
            filters['partners'] = ''
    ...
    def prepare_hierarchy(self, move_lines):
        ...
        if move_lines:
            ...
            for line in move_lines:
                ...
                try:
                    account_id = self.env['account.account'].search(
                        [('code', '=', q['code'][:8])], limit=1)
                except:
                    account_id = False
                if account_id:
                    if account_id.code != str(tmp['code']):
                        tmp.update(prepare_tmp(id=str(tmp['id']) + 'z5',
                                               code=str(tmp['code'])[:8],
                                               name=str(
                            account_id.name if account_id else ''),
                            indent_list=[1],
                            parent=parent_1 + parent_2 + parent_3 + parent_4))

                        if tmp['code'] not in [k['code'] for k in hirarchy_list]:
                            hirarchy_list.append(tmp)
                            parent_5 = [tmp['id']]
                final_parent = ['0'] + parent_1 + \
                    parent_2 + parent_3 + parent_4 + parent_5
                tmp = q.copy()
                tmp.update({'account_id': str(tmp['id']), 'code': str(tmp['code']), 'parent': ' a'.join(
                    final_parent), 'dummy': False, 'indent_list': [1], 'partner_id': tmp['partner_id'], 'partner_name': tmp['partner_name'], 'partner_vat': tmp['partner_vat']})
                hirarchy_list.append(tmp)
    ...
    def process_data(self, data):
        if data:
            ...
            where_clause = f'aml.company_id = {company_id.id}'
            if data.get('account_ids', []):
                where_clause += ' AND aa.id IN %s' % str(
                    tuple(data.get('account_ids'))+tuple([0]))

            if data.get('partner_ids', []):
                where_clause += ' AND aml.partner_id IN %s' % str(
                    tuple(data.get('partner_ids'))+tuple([0]))

            account_partner_sql = f"""
                SELECT aa.code, aa.id account_id, aml.partner_id
                FROM account_move_line aml
                JOIN account_account aa ON aa.id = aml.account_id
                WHERE {where_clause}
                GROUP BY
                    aa.code, aa.id, aml.partner_id
                ORDER BY aa.code
            """

            cr.execute(account_partner_sql)
            account_partner = cr.dictfetchall()

            lines = {}

            # base for accounts to display
            for acc_part in account_partner:
                account_id = account_obj.browse(
                    acc_part.get('account_id', False) or False)
                partner_id = partner_obj.browse(
                    acc_part.get('partner_id', False) or False)
                values = {'name': account_id.name, 'code': account_id.code, 'id': account_id.id,
                          'partner_id': partner_id.id, 'partner_vat': partner_id.vat or '', 'partner_name': partner_id.name or '',
                ...
                lines.update(
                    {'%s-%s' % (account_id.code, partner_id.id): values})
            ...
            total_end_bal = 0.0
            cont = len(lines)
            move_lines = lines.copy()
            for key, vals in lines.items():
                cont -= 1
                account = account_obj.browse(vals.get('id', False) or False)
                partner = partner_obj.browse(
                    vals.get('partner_id', False) or False)
                WHERE_INIT = WHERE + \
                ...
                if partner:
                    ...
                    WHERE_INIT += " AND l.partner_id IS NULL"
                init_blns = 0.0
                deb = 0.0
                cre = 0.0
                end_blns = 0.0
                ...
                move_lines[key]['initial_balance'] = init_blns['initial_balance']
                move_lines[key]['initial_debit'] = init_blns['initial_debit']
                move_lines[key]['initial_credit'] = init_blns['initial_credit']

                if account.user_type_id.include_initial_balance and self.strict_range:
                    move_lines[key]['initial_balance'] = 0.0
                    move_lines[key]['initial_debit'] = 0.0
                    move_lines[key]['initial_credit'] = 0.0
                ...
                if partner:
                    WHERE_CURRENT += " AND l.partner_id = %s" % partner.id
                else:
                    WHERE_CURRENT += " AND l.partner_id IS NULL"
                sql = ('''
                    SELECT
                ...
                cre = op['credit']
                bln = op['balance']
                move_lines[key]['debit'] = deb
                move_lines[key]['credit'] = cre
                move_lines[key]['balance'] = bln

                end_blns = init_blns['initial_balance'] + bln
                end_cr = init_blns['initial_credit'] + cre
                end_dr = init_blns['initial_debit'] + deb

                move_lines[key]['ending_balance'] = end_blns
                move_lines[key]['ending_credit'] = end_cr
                move_lines[key]['ending_debit'] = end_dr
    ...
    def get_filters(self, default_filters={}):
        ...
        filter_dict = {
            ...
            'journals_list': [(p.id, p.name) for p in partners],
    ...
    def action_pdf(self):
        ...
        return self.env.ref(
            ...
            '.action_print_trial_balance_partner').with_context(landscape=False).report_action(
    ...
    def action_xlsx(self):
        ...
        return {
            ...
            'data': {'model': 'ins.trial.balance.partners',
    ...
    def get_xlsx_report(self, data, response):
        ...
        # Get record and data
        record = self.env['ins.trial.balance.partners'].browse(
        ...
        row_pos_2 = 0
        row_pos = 0
        sheet.merge_range(0, 0, 0, 10, 'Balance de prueba por tercero' +
        ...
        row_pos += 3
        sheet.merge_range(row_pos, 3, row_pos, 5,
                          'Balance Inicial', format_merged_header)
        datestring = fields.Date.from_string(
            str(filter.get('date_from'))).strftime(lang_id.date_format)
        sheet.write(row_pos, 6, datestring,
                    format_merged_header_without_border)
        sheet.write(row_pos, 7, _(' To '), format_merged_header_without_border)
        datestring = fields.Date.from_string(
            str(filter.get('date_to'))).strftime(lang_id.date_format)
        sheet.write(row_pos, 8, datestring,
                    format_merged_header_without_border)
        sheet.merge_range(row_pos, 9, row_pos, 11,
                          'Balance final', format_merged_header)
        row_pos += 1
        sheet.write(row_pos, 0, _('Cuenta'), format_header)
        sheet.write(row_pos, 1, _('NIT/CC'), format_header)
        sheet.write(row_pos, 2, _('Tercero'), format_header)
        sheet.write(row_pos, 3, _('Debito'), format_header)
        sheet.write(row_pos, 4, _('Credito'), format_header)
        sheet.write(row_pos, 5, _('Balance'), format_header)
        sheet.write(row_pos, 6, _('Debito'), format_header)
        sheet.write(row_pos, 7, _('Credito'), format_header)
        sheet.write(row_pos, 8, _('Balance'), format_header)
        sheet.write(row_pos, 9, _('Debito'), format_header)
        sheet.write(row_pos, 10, _('Credito'), format_header)
        sheet.write(row_pos, 11, _('Balance'), format_header)
        ...
        if account_lines:
            if not filter.get('show_hierarchy'):
                for line in account_lines:  # Normal lines
                    ...
                    sheet.write(row_pos, 1, account_lines[line].get('partner_vat') ,
                                line_header_light_left)
                    sheet.write(row_pos, 2, account_lines[line].get('partner_name') ,
                                line_header_light_left)
                    sheet.write(row_pos, 3, float(account_lines[line].get('initial_debit')),
                                line_header_light)
                    sheet.write(row_pos, 4, float(account_lines[line].get('initial_credit')),
                                line_header_light)
                    sheet.write(row_pos, 5, float(account_lines[line].get('initial_balance')),
                                line_header_highlight)
                    sheet.write(row_pos, 6, float(account_lines[line].get('debit')),
                                line_header_light)
                    sheet.write(row_pos, 7, float(account_lines[line].get('credit')),
                                line_header_light)
                    sheet.write(row_pos, 8, float(account_lines[line].get('balance')),
                                line_header_highlight)
                    sheet.write(row_pos, 9, float(account_lines[line].get('ending_debit')),
                                line_header_light)
                    sheet.write(row_pos, 10, float(account_lines[line].get('ending_credit')),
                                line_header_light)
                    sheet.write(row_pos, 11, float(account_lines[line].get('ending_balance')),
                                line_header_highlight)
            else:
                for line in account_lines:  # Normal lines
                    row_pos += 1
                    ...
                    if line.get('dummy'):
                        sheet.write(row_pos, 0, blank_space + line.get('code') + ' ' + line.get('name'),
                                    line_header_light_left)
                    else:
                        sheet.write(row_pos, 0,
                                    blank_space +
                                    ...
                                    line_header_light_left)
                    sheet.write(row_pos, 1,  line.get('partner_vat'),
                                    line_header_light_left)
                    sheet.write(row_pos, 2,  line.get('partner_name'),
                                    line_header_light_left)
                    sheet.write(row_pos, 3, float(
                        line.get('initial_debit')), line_header_light)
                    sheet.write(row_pos, 4, float(
                        line.get('initial_credit')), line_header_light)
                    sheet.write(row_pos, 5, float(line.get('initial_balance')),
                                line_header_highlight)
                    ...
                    sheet.write(row_pos, 10, float(
                        line.get('ending_credit')), line_header_light)
                    sheet.write(row_pos, 11, float(line.get('ending_balance')),
                                line_header_highlight)
            ...
            if filter.get('strict_range'):
                ...
                sheet.write(row_pos, 0, '        ' + retained['RETAINED'].get('name'),
                                        line_header_light_left)
                sheet.write(row_pos, 1,  retained['RETAINED'].get('partner_vat'),
                                        line_header_light_left)
                sheet.write(row_pos, 2,  retained['RETAINED'].get('partner_name'),
                                        line_header_light_left)
                sheet.write(row_pos, 3, float(retained['RETAINED'].get('initial_debit')),
                            line_header_light)
                sheet.write(row_pos, 4, float(retained['RETAINED'].get('initial_credit')),
                            line_header_light)
                sheet.write(row_pos, 5, float(retained['RETAINED'].get('initial_balance')),
                            line_header_highlight)
                sheet.write(row_pos, 6, float(retained['RETAINED'].get('debit')),
                            line_header_light)
                sheet.write(row_pos, 7, float(retained['RETAINED'].get('credit')),
                            line_header_light)
                sheet.write(row_pos, 8, float(retained['RETAINED'].get('balance')),
                            line_header_highlight)
                sheet.write(row_pos, 9, float(retained['RETAINED'].get('ending_debit')),
                            line_header_light)
                sheet.write(row_pos, 10, float(retained['RETAINED'].get('ending_credit')),
                            line_header_light)
                sheet.write(row_pos, 11, float(retained['RETAINED'].get('ending_balance')),
                            line_header_highlight)
            # Sub total line
            row_pos += 2
            sheet.write(row_pos, 0,
            ...
            sheet.write(row_pos, 2, '',
                        line_header_left_total)
            sheet.write(row_pos, 3, float(subtotal['SUBTOTAL'].get('initial_debit')),
                        line_header_light_total)
            sheet.write(row_pos, 4, float(subtotal['SUBTOTAL'].get('initial_credit')),
                        line_header_light_total)
            sheet.write(row_pos, 5, float(subtotal['SUBTOTAL'].get('initial_balance')),
                        line_header_total)
            sheet.write(row_pos, 6, float(subtotal['SUBTOTAL'].get('debit')),
                        line_header_light_total)
            sheet.write(row_pos, 7, float(subtotal['SUBTOTAL'].get('credit')),
                        line_header_light_total)
            sheet.write(row_pos, 8, float(
                subtotal['SUBTOTAL'].get('balance')), line_header_total)
            sheet.write(row_pos, 9, float(subtotal['SUBTOTAL'].get('ending_debit')),
                        line_header_light_total)
            sheet.write(row_pos, 10, float(subtotal['SUBTOTAL'].get('ending_credit')),
                        line_header_light_total)
            sheet.write(row_pos, 11, float(subtotal['SUBTOTAL'].get('ending_balance')),
                        line_header_total)
